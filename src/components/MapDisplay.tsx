...
import { getRouteForDisplay, convertStopsToMarker } from '../lib/mapDisplay';
import '../index.css';
 ...
 */
export default function MapDisplay({stops} : MapDisplayProps) {
    ...
    useEffect(() => {
        ...
        if (stops && (stops.length >= 2)) {
            const response = getRouteForDisplay(stops, false)
            response.then((value : {[key: string] : any}) => {
                const data = value.routes[0];
